...
const getMiddlewareInstance = module.exports = function (opts) {
  ...
  const expeditious = require('expeditious')({
    ...
    engine: opts.engine || (function () {
      ...
      if (!warnedAboutMemoryCache) {
        ...
        console.warn(
          'express-expeditious: You did not supply an engine. Using default memory engine. If you are not carefull, there may be memmory issuse.'
        )
      }
  ...
  const expeditiousMiddleware = function (req, res, next) {
    ...
    if (res.socket == null) {
      log(
        'skipping cache. res.socket is null',
        cacheKey
      );
      next();
      return;
